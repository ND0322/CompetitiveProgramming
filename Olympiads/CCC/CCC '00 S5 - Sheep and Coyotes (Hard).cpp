...
typedef pair<pair<long long,long long>,long long> line;
...
int main(){
    ...
    for(int i = 0; i < n; i++){
        ...
        sheep.push_back({{-2 * x[i], x[i]*x[i] + y[i]*y[i]}, i});
    }
    ...
    for(int i = 0; i < lines.size(); i++){
       ...
       cout << "The sheep at (" << x[lines[i].second] << ", " << y[lines[i].second] << ") might be eaten.\n";

       //out of bounds
       if(i != lines.size()-1 && intersect(lines[i], lines[i+1]) > 1e6) break;
    }
